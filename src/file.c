...
void GetDirName(char *fname,char *dname){
...
i=n;
  if(i+1<MAX_WORD_LEN){
    strncpy(dname,fname,(size_t)(i+1));
    strncpy(&dname[(size_t)(i+1)],"\0",1);
  }
  else{
    fprintf(stderr,"Dir name too long, %d. Max. length: %d characters\n",i+1,MAX_WORD_LEN);
    exit(EXIT_FAILURE);
  ...
  */
  ...
  do{
    if(fscanf(fp,"%c",&s)!=EOF){
      switch(s){ 
      case '\n': /* end of line */
	if(nchar==0)endblock++;  /* end of block? */
	else{
	  nlines++;
	}
	comment=0;
...
int AnalizeLine(char *cad){
  ...
  */
  ...
  for(i=0;i<(int)len && i< MAX_LINE_LEN;i++){
    /* look for a letter */
    if(strncmp(&cad[i],"a",1)>=0 && strncmp(&cad[i],"z",1)<=0)return(COMMENT);
    if(strncmp(&cad[i],"A",1)>=0 && strncmp(&cad[i],"Z",1)<=0)return(COMMENT);
    /* --look for a letter */
    if(status==0){
      switch (cad[i]){
...
int ReadNBlocks(char *fname,struct Block *br,long fpos){
  ...
  */
  ...
  if(fseek(fp,fpos,SEEK_SET)==-1)printf("ERROR Seeking file\n");
...
int TakeWordFromCad(char *word,char *cad){
  ...
  */
  ...
  strcpy(tmp,"");
  if(ReadWordFromCad(word,cad)==0)return(-1);
  ...
  */ 
  ...
  if(len){
	...
	if(cad[i]==' ' || cad[i]=='\t')cl++;
      ...
      if(swcr==0){
	if(cad[len-i-1]==' ' || cad[len-i-1]=='\t')cr++;
	else
	  swcr=1;
      }
      if(swcl==1 && swcr==1)break;
      if(len-cl-cr<=0)break;
    }
