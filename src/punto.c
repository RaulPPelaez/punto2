...
/* ************  PUNTO  M.R.H. 1998-2009 ******************
		Author: Mariano Revenga
		E-mail: mrevenga at users.sourceforge.net
		programa para visualizar moleculas 
		version 1.0.0.4 Jun 2009
****/


/* 
   time bin/punto -s 6  -w 1:1.01 -z 1:2:6 -B dat/plot.dat 
   (785 particulas, 608 pasos)
   
   todo:                14.719 s
   ReadBlock:           10.419 s 
   MovePuntos:           4.170 s
   
   punto 0.98pre1 todo: 10.32
...
int main(int argc, char *argv[],char **env)
{
  ...
  char dname[MAX_WORD_LEN]="";  /* name of the directory of the data file */
  int pos_columns[7];
  ...
  GetDirName(dfile.name,dname); // HERE not used
  ...
  max_nsize=MAX_NSIZE;
  if(option.size)max_nsize=param.size;
  if(max_nsize>MAX_NSIZE)max_nsize=MAX_NSIZE;
  
  ntrace=TRACE_DEFAULT;
  if(option.ntrace){
    ntrace=param.ntrace;
    if(ntrace>=TRACE_MAX)ntrace=TRACE_MAX;
    if(ntrace<0)ntrace=TRACE_DEFAULT;
  }
  ...
  /* order of columns  */
  OrderOfColumns(option,val_run.dimension,dfile.ncol,n_option_columns,pos_columns,&pos);
  /*  --order of columns */ 
  ...
  if((punto = (struct Punto *)calloc((size_t)max_N,sizeof(struct Punto)))==NULL){
  ...
  for(i=0;i<max_N;i++)tabla[i]=i;
  
  dfile.nblocks=0;
  if((block = (struct Block *)calloc((size_t)1,sizeof(struct Block)))==NULL){
    perror("calloc");exit(EXIT_FAILURE);}
  block[0].read=FALSE;
  ...
  if(option.sizebox==TRUE){
    if(GetBox(val_run.dimension,&param))option.sizebox=FALSE;
  }
  ...
  printf("\tnumber of columns of the data file: %d \n\tNumber of particles of the first block: %ld\n",
	 dfile.ncol,N);
  printf("dimension: %d\n",val_run.dimension);
  ...
  if((bola = 
      (struct Sprite *)malloc((size_t)(numsprites)*sizeof(struct Sprite )))==NULL){
  ...
  if(option.size)universe.psize=param.size;
  else(universe.psize=DEFAULTRADIO);
  ...
  if ( SDL_Init((Uint32)(SDL_INIT_VIDEO|SDL_INIT_NOPARACHUTE|SDL_INIT_TIMER)) < 0 ) {
  ...
  window.flags = (Uint32)(SDL_HWSURFACE|SDL_ANYFORMAT|SDL_RESIZABLE|SDL_DOUBLEBUF|SDL_ASYNCBLIT);
  ...
  if ( ! screen ) {
    fprintf(stderr, "Couldn't set %dx%d video mode: %s\n",
	    window.w, window.h, SDL_GetError());
    exit(EXIT_FAILURE);
  }
  ...
  if(option.gradient || option.real_color){
    ...
    for(i=0;i<MAX_NSCOLORS;i++){
      colortable[i]=SDL_MapRGB(screen->format,
			       rgbcolortable[41+i*8].r,
			       rgbcolortable[41+i*8].g,
			       rgbcolortable[41+i*8].b);
    }
  ...
  if(universe.escale.x>universe.escale.y)universe.escale.x=universe.escale.y;
  universe.escale.y=universe.escale.x;
  
  if(option.verbose>1) 
    printf("escala_x: %g esc_y:%g zoom:%g \n",universe.escale.x,universe.escale.y,universe.zoom);
  
  
  universe.dshift.x=(val_data.max_x-val_data.min_x)*.5*universe.escale.x;
  universe.dshift.y=(val_data.max_y-val_data.min_y)*.5*universe.escale.x;
  ...
  if(option.radio==FALSE){
    bola[0].surface=CreateBall3D(screen,defaultcolor,
				 (universe.psize+1)*(universe.psize<MAX_NSIZE)+MAX_NSIZE*(universe.psize>MAX_NSIZE) );
    //    printf("optionradio:%d universe.psize: %d spr:%p\n",option.radio,universe.psize,bola[0].surface);
  ...
  for(i=0;i<ntrace;i++){
    if((traceball[i] = (struct Punto *)calloc((size_t)1,sizeof(struct Punto)))==NULL){
  ...
  while ( !done ) {
    ...
    if(!delay && option.anim==TRUE)delay=val_run.delay*10;
    
    if(cont!=0){
      
      if(((option.anim==TRUE && kEOF==FALSE) || kp.space==TRUE || kp.pressed) ){  /* animation */
	
	do{
	  status=CheckEvent(&event); /* look for events, if exists, puts the first in event */
	  time=SDL_GetTicks();
	  if(time-time0-delay+10>0){
	    ...
	    if(delay && option.pausa==TRUE && option.anim==TRUE && option.fast==FALSE)SDL_Delay(10); 
	  }
	}
	...
	option.pausa=TRUE;
      ...
      if(kp.minus==TRUE){
	val_run.delay-=1+val_run.delay/10;
	if(val_run.delay<0)val_run.delay=0;
	PurgeSprites(bola,1);
      }
	...
	option.trace++;
	if(option.trace>2)option.trace=0;
	...
	kp.f++;
      ...
      if(kp.i==TRUE){
	if(kp.shift)universe.zoom_size*=1.05;
	else     
      ...
      if(kp.user){
	kp.user=FALSE;
	if(option.verbose>1)printf("Checking file size\n");
	...
	if(filesize!=dfile.size){
      ...
      if(option.mode==ZOOM){
	if(kp.z==TRUE){
	  if(kp.shift)universe.zoom*=1.05;
	  else     
	    universe.zoom/=1.05;
	}
      }
	...
	if(kp.less) 
	  shift=-.001*lx;
	if(kp.z==TRUE){
	  if(kp.shift){universe.cv.z+=shift;universe.zoom/=(1+shift/universe.cv.z);}
	  else{
	    universe.cv.z-=shift;
	...
	window.shift.y-=ishift/universe.zoom;
      ...
      if(kp.less)inc_ang2/=10;
      if(universe.zoom>1)inc_ang2/=universe.zoom;
      if(kp.k_7) 
	...
	universe.ang.z -= inc_ang2;
      ...
      if(universe.ang.x||universe.ang.y||universe.ang.z)option.rotate=TRUE;
	...
	(void)SDL_GetMouseState(&mu_x, &mu_y);
      ...
      if(md_x-filerect.x+20>0 && (filerect.x+filerect.l+20)-md_x >0){
	if(md_y-filerect.y>0 && (filerect.y+filerect.h)-md_y >0){
      ...
      if(kp.mbup==TRUE)kp.mclick=FALSE;
    }
    /* --mouse click handle */
    
    if(option.reverse==TRUE)kEOF=FALSE;
    
    if(actualizar==TRUE){
      if((option.anim && delay==0) || kp.space==TRUE){
	if(option.reverse){
	  nextblock--;
	  if(nextblock<0){nextblock=0;option.anim=FALSE;}
	}
	else{
	  if(kEOF==FALSE)nextblock++;
	}
      }
      if(kp.pageup==TRUE){
	nextblock+=10;
    ...
    if(kEOF==FALSE){
      ...
      if(inmouse)nextblock=NextBlock((float)(md_x-filerect.x)/filerect.l,dfile.name);
      md_x=0;
...
void DrawInfo(SDL_Surface *screen, struct Window win,struct Universe u,struct DataFile df){
//void DrawInfo(SDL_Surface *screen, struct Window win,struct Universe u,Uint32 background){
  //  int mouse_x,mouse_y;
  int x;
  float y;
  int fs=FONTSIZE;
  ...
  if(option.fast==FALSE){
    //    if(option.comment==TRUE) x=DrawText(screen,comment,5,win.h-20,ledcolor);
    if(option.real_color)DrawScale(screen,colortable,ledcolor,val_run,10,60);
  }
}
...
void MovePuntos(struct Universe u,struct Parametres par,struct Punto *p){
  ...
  if(cont==0){ /* initialization */
    long i;
    for(i=0;i<numactivados;i++)tabla[i]=i;
    cont++;
  }
  ...
  if(option.number==TRUE){
    ...
    n2=par.N2;
    if(n1<0)n1=0;
    if(n2>numactivados)n2=numactivados;
  }
  
  if((actualizar==TRUE || option.rotate==TRUE)){
    struct Point c;
    c=u.cm;
    if(option.mode==NAV)c=u.cv;
...
void DrawAll(SDL_Surface *screen, struct Window win,struct Universe u,struct Parametres par,Uint32 background,struct DataFile df){
  ...
  if(actualizar==TRUE){
    ...
    if(option.fast==FALSE){
      //      printf("%f %f\n",u.escale.x,u.zoom);
      DrawPuntos(screen,win,u,punto,tabla,u.cw,u.escale.x*u.zoom,par,val_run);
      
      if(option.periodic==TRUE){
	DrawPeriodicPuntos(screen,win,u,punto,tabla,u.cw,u.escale.x*u.zoom,par,val_run);
      }
    ...
    if(option.comment==TRUE) DrawText(screen,comment,5,win.h-20,ledcolor);
    
    if(option.verbose>1)printf("sprites active: %d\n",
		...
		struct Parametres par,struct Values data)
{
  ...
  for ( i=0; i<numactivados; i++ ) {
    if(t[i]<0 || t[i]>max_N+1){
      printf("FATAL ERROR: seg fault. exiting...\n\t  KK 10: %d t_i %ld max_N: %ld\n",i,t[i],max_N);
      exit(EXIT_FAILURE);
    }
    
    
    p[t[i]].sprite=NULL;
    if(t[i]<n1 || t[i]>=n2)continue;
    x=(float)(p[t[i]].x1*z+cwin.x);
    y=(float)(w.h-((p[t[i]].y1)*z+cwin.y));
    
    if((int)x>max || ((int)y>max) || ((int)x<-max_nsize) || ((int)y<-max_nsize)){
      continue;
    }
    if(option.mode==NAV) if(p[t[i]].z1>0)continue;
    coloridx=0;
	...
	if(coloridx>MAX_NSCOLORS-1)coloridx=MAX_NSCOLORS-1;
      } 
    ...
    if(option.radio==TRUE && option.varfield==TRUE){
      ...
      if(radio>0){
	if(val_run.max_r>0){
	  if(radio>val_run.max_r)radio=val_run.max_r;
	}
	//	printf("%f %f %f\n",radio,z,u.zoom_size);
    ...
    if(sizeidx>max_nsize)sizeidx=max_nsize;
    ...
    //     printf("n: %d  IDX:%d option_type:%d size_idx:%d\n",i,idx,option.type,sizeidx);
    bola[idx].cont=Cont;
    bola[idx].time=time;
	...
	if(option.verbose>1){
	  printf(" (%d) %d+%d=%d ",idx,sizeidx,coloridx,idx);fflush(stdout);
	}
	...
	switch(option.type){
	case CIRCLE:
	  bola[idx].surface=CreateBall(screen,fgcolor,defaultcolor,sizeidx);
	  break;
	case SQUARE:
	  bola[idx].surface=CreateRectangle(screen,fgcolor,fgcolor,2*sizeidx+1);
	  break;
	case SPHERE:
	  bola[idx].surface=CreateBall3D(screen,fgcolor,sizeidx);
	  break;
	default:
	  bola[idx].surface=CreateBall(screen,defaultcolor,fgcolor,sizeidx);
  ...
  if(option.verbose>1)printf("m: %ldkB t:%d z:%f\n",total_memory/1024,(int)(time/1000),z);
...
void DrawTracePuntos(SDL_Surface *screen,struct Window w,struct Universe u,struct Parametres par,float z,int order){
  ...
  if(order==0){ /* freeing memory */
    ...
    trace_cont=1;
    trace_num=1;
    if(option.verbose>0)printf("FREEING MEM (trace points)\n");
    for(i=0;i<ntrace;i++){
      if((traceball[i] = (struct Punto *)realloc(traceball[i],
						 (size_t)trace_num*sizeof(struct Punto)))==NULL){
	perror("realloc");exit(EXIT_FAILURE);
      }
    }    
    return;
  ...
  if(nextblock!=block){
    if(nextblock-block==1 || block-nextblock==1){
      trace_pos++; 
      if(trace_pos>=ntrace)trace_pos=0; 
      trace_cont++; 
      if(trace_cont>=ntrace)trace_cont=ntrace; 
    }
    else{
  ...
  if(numactivados>trace_num){
    ...
    if(option.verbose>1)printf("REALLOCATING\n");
    for(i=0;i<ntrace;i++){
      if((traceball[i] = 
	  (struct Punto *)realloc(traceball[i],
				  (size_t)numactivados*sizeof(struct Punto)))==NULL){
	perror("realloc");exit(EXIT_FAILURE);
      }
  ...
  for(i=n1;i<n2;i++){
    for(j=0;j<trace_cont;j++){
      ...
      if(option.trace==1)
	PutPixel(screen,(int)(traceball[k][i].x1*z+u.cw.x),(int)(w.h-(traceball[k][i].y1*z+u.cw.y)),ledcolor);
      else{
	if(j < trace_cont-1){
	  l=trace_pos-j-1;
	  if(l<0)l+=ntrace;
	  if(fabs(traceball[l][i].x-traceball[k][i].x)<lenx){
	    if(fabs(traceball[l][i].y-traceball[k][i].y)<leny){
	      DrawLine(screen,
		       (int)(traceball[l][i].x1*z + u.cw.x) ,(int)( w.h-(traceball[l][i].y1*z + u.cw.y)),
		       (int)(traceball[k][i].x1*z + u.cw.x) ,(int)( w.h-(traceball[k][i].y1*z + u.cw.y)),
		       ledcolor);
	    }
	  }
	}
		       ...
		       struct Parametres par,struct Values data)
{
  ...
  struct Point c;
  c=u.cm;
  if((p0 = (struct Punto *)malloc((size_t)numactivados*sizeof(struct Punto)))==NULL){
    perror("malloc");exit(EXIT_FAILURE);}
  ...
  for(l=-1;l<2;l++){
    if(val_run.dimension==2)l=0;
    for(j=-1;j<2;j++){
      for(k=-1;k<2;k++){
	if(k==0  && j==0 && l==0)continue;
	for(i=0;i<numactivados;i++){
...
void  RangeOfValues(struct Punto *p,struct Options opt,struct Values *data){
  ...
  */
  ...
  for(i=0;i<numactivados;i++){
    if(opt.radio){ 
      if(p[i].radio > data->max_r) data->max_r=p[i].radio;
      if(p[i].radio < data->min_r) data->min_r=p[i].radio;
    } 
    if(opt.color){
      if(p[i].color > data->max_c) data->max_c=p[i].color;
      if(p[i].color < data->min_c) data->min_c=p[i].color;
    }
    if(opt.field==TRUE){
      if(p[i].vx > data->max_f)data->max_f=p[i].vx;
      if(p[i].vy > data->max_f)data->max_f=p[i].vy;
      if(p[i].vz > data->max_f)data->max_f=p[i].vz;
      
      if(p[i].vx < data->min_f)data->min_f=p[i].vx;
      if(p[i].vy < data->min_f)data->min_f=p[i].vy;
      if(p[i].vz < data->min_f)data->min_f=p[i].vz;
    }
    if(p[i].x > data->max_x)data->max_x=p[i].x;
    if(p[i].x < data->min_x)data->min_x=p[i].x;
    if(p[i].y > data->max_y)data->max_y=p[i].y;
    if(p[i].y < data->min_y)data->min_y=p[i].y;
    if(p[i].z > data->max_z)data->max_z=p[i].z;
    if(p[i].z < data->min_z)data->min_z=p[i].z;
  }
...
void InfoSprites(struct Sprite *b,struct Options opt){
  ...
  for(i=0;i<numsprites;i++){ 
    if(b[i].on==TRUE){  
      ...
      if(sw==0){minpos=(long)b[i].surface;sw=1;}
      if ((long)b[i].surface<minpos)minpos=( long)b[i].surface;
  ...
  if(mem){
    if(option.verbose){
      ...
      printf("MAXPOS: %ld MINPOS: %ld dif: %ld\n",maxpos,minpos,maxpos-minpos);
      printf("factor: %f\n",((float)(maxpos-minpos))/(float)mem);
    }
    if(option.trace)mem+=numactivados*ntrace*sizeof(struct Punto);
...
void PurgeSprites(struct Sprite *b,int type){
  ...
  case 0:
    for(i=0;i<numsprites;i++){ 
      ...
      if(b[i].on==TRUE     &&  (b[i].cont < (Cont-20))  /*        && b[i].time < mintime */    ){  
	b[i].on=FALSE; 
	b[i].cont=0; 
	b[i].time=0;
	SDL_FreeSurface(b[i].surface); 
	b[i].surface=NULL;
	if(option.verbose>1)printf(" (%d)<%d+%d=%d ",i,i%max_nsize,(int)(i/max_nsize),i);
	//	total_memory-=(i%max_nsize)*(i%max_nsize)*16;
	total_memory-=((float)i/MAX_NSCOLORS)*((float)i/MAX_NSCOLORS)*16;
	cont1++;
      }
      else {
	cont2++;
  ...
  case 1:
	...
	SDL_FreeSurface(b[i].surface); 
	b[i].surface=NULL;
	if(option.verbose>1)printf(" <(%d) %d+%d ",i,i%max_nsize,(int)(i/max_nsize));
	total_memory-=(i%max_nsize)*(i%max_nsize)*16;
      }
    } 
    break;
  ...
  default:
	...
	SDL_FreeSurface(b[i].surface); 
	b[i].surface=NULL;
	if(option.verbose>1)printf(" <%d+%d ",i%max_nsize,(int)(i/max_nsize));
	total_memory-=(i%max_nsize)*(i%max_nsize)*16;
      } 
    }
    break;
...
void Reescal_Color(int N,struct Punto *p1,struct Punto *p,float max_nc,float min_color,float color_factor)
{
  int i;
  for(i=0;i<N;i++){
...
struct Punto *ReadBlock(int *next,struct DataFile df,struct PosCol pos,long *numact,struct Punto *p,int *status)
{
  /*
  ...
  *status=TRUE;
  ...
  if((temp = (double *)malloc((size_t)(df.ncol+1)*sizeof(double)))==NULL){
    perror("malloc");exit(EXIT_FAILURE);}
  ...
  if((*next-lastblock!=1   || maxblock<=lastblock )){ 
    if(fseek(df.fp,b->pos,SEEK_SET)==-1)printf("ERROR Seeking file\n"); 
  } 
  ...
  do{
	...
	if(rvalue==EOF){
	  kEOF=TRUE;printf("ERROR-readblock-fscanf\n");exit(EXIT_FAILURE);
	}
	while(line[k]!=' ' && line[k]!='\t' && k<lenline)k++;
	k++;
      }
      p[i].x=temp[pos.x];
      p[i].y=temp[pos.y];
	...
	if((p = (struct Punto *)realloc(p,(size_t)(i+1024)*sizeof(struct Punto)))==NULL){
	  perror("realloc");exit(EXIT_FAILURE);
	}
	max_N=i+1023;
	reallocpunto=TRUE;
  ...
  if(reallocpunto){
    if((tabla = (long *)realloc(tabla,(size_t)(max_N+1)*sizeof(long)))==NULL){
      perror("realloc");exit(EXIT_FAILURE);}
    //printf("asignando %ld a tabla. numact: %ld\n",max_N,*numact);
    for(i=*numact;i<(size_t)max_N+1;i++)tabla[i]=i;
  }
  
  free(temp);
  lastblock=*next;
  if(kEOF==TRUE){
    if(!maxblock)maxblock=lastblock;
  }
...
int Arguments(int argc,char *argv[],struct Options *opt,struct Parametres *par)
/* function Arguments 
  ...
  
{
  ...
  char *pointer;
  char *endptr=NULL;
  static char str[MAX_WORD_LEN]="",str0[MAX_WORD_LEN]=""; /* str0[] is used for deleting str[] */
  int flag=0;
  int narg;
  char arg[MAX_WORD_LEN]="";
  char pal[MAX_WORD_LEN]="";
  char *validargum[]={"h","s","size","n","g","geom","geometry","d","r","radio",
  ...
  for(i=0;i<argc;i++){
	...
	if(strncmp("t",arg,MAX_WORD_LEN)==0){ // type of punto
	  ...
	  if(opt->type==0 && argv[i+1]==endptr){
	    ...
	    fprintf(stderr,"Warning: -t option must have an argument. Setting default.\n");
	  }
	}
	...
	if(strncmp("trace",arg,MAX_WORD_LEN)==0){ // trace points on
	  ...
	  if(par->ntrace>0 && par->ntrace<=TRACE_MAX)opt->ntrace=TRUE;
	}
	break;
      case 's': 
	if(strncmp("s",arg,MAX_WORD_LEN)==0 || strncmp("size",arg,MAX_WORD_LEN)==0){  // size of puntos
	  opt->size=TRUE;
	  par->size=strtol(argv[i+1],&endptr,10);
	  if(par->size==0 && argv[i+1]==endptr){
	    opt->size=FALSE;
	    fprintf(stderr,"Warning: -s option must have an argument. Setting default.\n");
	  }
	}
	break;
      case 'h':
	...
	if(strncmp("n",arg,MAX_WORD_LEN)==0){  // selected puntos
	  ...
	  if(sw==0){
	    ...
	    fprintf(stderr,"Warning: -n option bad formed. Plotting all puntos.\n");
	  }
	}
	break;
      case 'g':
	...
	if(strncmp("g",arg,MAX_WORD_LEN)==0 ||   // geometry of the window
	  ...
	  if(sw==0){
	    ...
	    fprintf(stderr,"Warning: -g option bad formed. Using default values.\n");
	  }
	}
	break;
      case 'd':
	if(strncmp("d",arg,MAX_WORD_LEN)==0){   // delay between frames
	  ...
	  if(par->pausa==0 && argv[i+1]==endptr){
	      opt->pausa=FALSE;
	      fprintf(stderr,"Warning: -d option must have an argument. Setting default.\n");
	  }
	  else{
	    printf("pausa: %d\n",(int)par->pausa);
	  }
	...
	break;
      case 'w': 
      case 'G':  
	if(strncmp("G",arg,MAX_WORD_LEN)==0||strncmp("w",arg,MAX_WORD_LEN)==0){  // Gradient scale
	  strncpy(str,str0,MAX_WORD_LEN);
	  strncpy(&str[MAX_WORD_LEN-1],"\0",(size_t)1);	  
	  pointer=strchr(argv[i+1],':');
	...
	if(strncmp("p",arg,MAX_WORD_LEN)==0){  // choose color palette
	  ...
	  if(opt->palette==0 && argv[i+1]==endptr){
	    ...
	    fprintf(stderr,"Warning: -p option must have an argument. Setting default.\n");
	  }
	}
	break;
      case 'V': 
	if(strncmp("V",arg,MAX_WORD_LEN)==0){  // Vector field
	  ...
	  if(pointer){
	    ...
	    if(par->min_f==0 && argv[i+1]==endptr)sw++;
	    par->max_f=(float)strtod(pointer+1,&endptr);
	    if(par->max_f==0 && pointer+1==endptr)sw++;
	    if(sw==0){
	      ...
	      fprintf(stderr,"Warning: -V option bad formed. Using default values.\n"); 
	    }
	    i++;
	  }
	  opt->field=TRUE;
	...
	break;
      case 'F':
	if(strncmp("F",arg,MAX_WORD_LEN)==0){  // root file name to save pixmaps
	  opt->file=TRUE;
...
void  SetValues(struct Values val_d,struct Parametres par,struct Options opt,struct Values *val){
  ...
  */
  ...
  if(opt.real_color==TRUE){
    if(opt.verbose>1)printf("real color\n");
...
int SetDimension(struct Parametres par,struct Options opt,int ncol,int noptcol){
  ...
  */
  ...
  d=3;
  if(opt.dimension)d=par.dimension;
  
  else{
    if(opt.column)ncol=noptcol;
    ...
    if(opt.field){
      ...
      if(ncol - num_options  >= 6)d=3;
      else if(ncol - num_options >= 4)d=2;
      else if(ncol - num_options >= 2)d=1;
      else{
...
void OrderOfColumns(struct Options opt,int d,int ncol,int noptc,int *posc,struct PosCol *p){
  ...
  if(opt.column==FALSE){
    ...
    if(d==3)
      p->z=idx++;
    if(opt.radio)p->r=idx++;
    if(opt.color)p->c=idx++;
    if(opt.field){
      if(d>0)
	p->vx=idx++;
      if(d>1)
    ...
    if(d*(1+(opt.field==TRUE))+opt.radio+opt.color != noptc){
      ...
      fprintf(stderr,"\tyou must specified %d columns\n",
	      d*(1+(opt.field==TRUE))+(opt.radio==TRUE)+(opt.color==TRUE));
    ...
    if(opt.radio==TRUE){p->r=posc[idx]-1;idx++;}
    if(opt.color==TRUE){p->c=posc[idx]-1;idx++;}
...
void BoxProperties(struct Values val_d,struct Parametres par,struct Options opt,struct Punto *b){
  /*
